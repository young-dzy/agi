...
type LocalSandbox struct {
	cfg       SandboxConfig
	validator *Validator // 内置校验，只允许 Safe 级别
}

// NewLocalSandbox 创建本地执行器
func NewLocalSandbox(cfg SandboxConfig) *LocalSandbox {
	// 本地模式：强制非白名单模式，但 block 规则仍然生效
	return &LocalSandbox{
		cfg:       cfg,
		validator: NewValidator(SecurityConfig{MaxCommandLength: cfg.MaxOutputBytes}),
	}
}
...
func (l *LocalSandbox) Exec(ctx context.Context, req ExecRequest) ExecResult {
	start := time.Now()
...
// Exec 返回模拟输出
func (m *MockSandbox) Exec(_ context.Context, req ExecRequest) ExecResult {
	return ExecResult{
		Command:  req.Command,
