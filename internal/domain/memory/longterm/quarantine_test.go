...
func TestQuarantine_BlocksRecall(t *testing.T) {
	...
	m.StoreClassified("用户城市: 北京", 0.9, []float64{1, 0, 0}, "preference", nil, "")
	m.StoreClassified("用户城市: 上海", 0.9, []float64{0, 1, 0}, "preference", nil, "")
	if m.Count() != 2 {
	...
	beforeIDs := idsOf(m.RecallByFilter("城市", queryEmb, RecallFilter{TopK: 10, MinScore: 0}))
	if len(beforeIDs) != 2 {
	...
	afterIDs := idsOf(m.RecallByFilter("城市", queryEmb, RecallFilter{TopK: 10, MinScore: 0}))
	if len(afterIDs) != 1 {
...
func TestQuarantine_IncludeFlag(t *testing.T) {
	...
	m.SetConsolidationConfig(DefaultConsolidationConfig())
	emb := []float64{1, 0, 0}
	m.StoreClassified("敏感信息", 0.9, emb, "fact", nil, "")
	id := m.LastID()
	m.Quarantine(id, "pii")

	// 默认不召回
	if got := m.RecallByFilter("敏感", emb, RecallFilter{TopK: 5}); len(got) != 0 {
		t.Errorf("默认不应召回, 得到 %d 条", len(got))
	}
	// IncludeQuarantined=true 时召回
	got := m.RecallByFilter("敏感", emb, RecallFilter{TopK: 5, IncludeQuarantined: true, MinScore: 0})
	if len(got) != 1 {
...
func TestQuarantine_FilterByCategory(t *testing.T) {
	m := New()
	m.StoreClassified("身份: 工程师", 0.9, nil, "identity", nil, "profile")
	m.StoreClassified("身份: 设计师", 0.9, nil, "identity", nil, "profile")
	if got := m.FilterByCategory([]string{"identity"}, 10); len(got) != 2 {
		t.Fatalf("应有 2 条 identity, 得到 %d", len(got))
	}
	m.Quarantine(m.LastID(), "test")
	if got := m.FilterByCategory([]string{"identity"}, 10); len(got) != 1 {
...
func TestQuarantine_QuarantinedItems(t *testing.T) {
	m := New()
	m.StoreClassified("a", 0.5, nil, "general", nil, "")
	m.StoreClassified("b", 0.5, nil, "general", nil, "")
	m.StoreClassified("c", 0.5, nil, "general", nil, "")
...
func TestUnquarantine(t *testing.T) {
	...
	m.SetConsolidationConfig(DefaultConsolidationConfig())
	emb := []float64{1, 0}
	m.StoreClassified("test", 0.9, emb, "general", nil, "")
	id := m.LastID()
	m.Quarantine(id, "x")
	if !m.Unquarantine(id) {
		t.Fatal("Unquarantine 应成功")
	}
	got := m.RecallByFilter("test", emb, RecallFilter{TopK: 5, MinScore: 0})
	if len(got) != 1 {
